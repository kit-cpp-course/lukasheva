...
{	
	...
	while (window.isOpen())
	{
		...
		if (count > 0)//Если есть хоть одна фигурка наверху то проигрываем
		{
		...
		scoretext.setString("Î÷êè:" + playerScoreString.str());//задаем строку тексту и вызываем сформированную выше строку методом .str()  
